...
class ListAction extends CommonAction {
    ...
    public function bookcase(){
		...
		$case=$model_bookcase->where("id={$caseid}")->field('aids,casename')->find();
		$where=array(
			'a.id'=>array('in',unserialize($case['aids']))
		);
		$this->channel=array('classname'=>$case['casename']);
		...
		$field=array(
			...
			$show       = $Page->show();// 分页显示输出
			 // 进行分页数据查询
			$list = M('xw')->table("tg_xw a")->join('tg_xwclass b on a.classid=b.id ')->cache(true,3600*24,'file')->where($where)->field($field)->limit($Page->firstRow.','.$Page->listRows)->order("a.id desc")->select();
			//tag标签
			foreach ($list as $key => $v) {
	    		$list[$key]['content']=msubstr(trim(strip_tags($v['content'])),0,100);
	  			$tagarr=explode('，', $v['tag']);
